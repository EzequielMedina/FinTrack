...
func (h *ChatHandler) Query(c *gin.Context) {
    ...
    if err := c.ShouldBindJSON(&req); err != nil {
        c.JSON(http.StatusBadRequest, gin.H{"error": "invalid body", "details": err.Error()}); return
    }
    ...
    if from.IsZero() && to.IsZero() {
        ...
        if !from.IsZero() { from = time.Date(from.Year(), from.Month(), from.Day(), 0, 0, 0, 0, from.Location()) }
        if !to.IsZero() { to = time.Date(to.Year(), to.Month(), to.Day(), 23, 59, 59, int(time.Second-time.Nanosecond), to.Location()) }
    }
    resp, err := h.svc.HandleQuery(c, ports.ChatQueryRequest{
        UserID: req.UserID, Message: req.Message, Filters: req.Filters, Period: ports.Period{From: from, To: to},
    })
...
func (h *ChatHandler) ReportChart(c *gin.Context) {
    ...
    if gb != "account" && gb != "accounts" && gb != "card" && gb != "cards" {
        c.JSON(http.StatusBadRequest, gin.H{"error": "groupBy inválido", "details": "Use 'account' o 'card'"}); return
    }
